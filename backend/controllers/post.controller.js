...
export const createpost = async(req,res)=> {
    try{
        ...
        if(!text && !img){
            return res.status(400)({error: "post musty have text or image"})
        }
        if(img){
...
try {
    ...
    if(post.user.toString() !== req.user._id.toString()){
        res.status(401).json({error : "you are not authorized to delete the post"})
    }
    if(post.img){
...
export const createcomment = async (req, res) => {
  try {
    ...
    // Only create a notification if the sender is not the recipient
    if (userId.toString() !== post.user.toString()) {
      const notification = new Notification({
        from: userId,
        to: post.user, // Ensure this matches your schema
        type: "comment",
